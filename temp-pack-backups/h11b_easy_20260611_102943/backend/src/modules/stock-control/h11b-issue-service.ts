...
export function previewH11BIssueService(payload: H11BPreviewRequest): H11BPreviewResponse {
  ...
  return {
    canFulfill: validation.length === 0 && shortageQty === 0,
...
export function createH11BIssueService(payload: H11BPreviewRequest): H11BIssueRecord {
  ...
  const hardValidation = preview.validation.filter(
    (message) => !message.startsWith('Insufficient stock')
  );

  if (hardValidation.length > 0) {
    throw new Error(hardValidation[0]);
  }
